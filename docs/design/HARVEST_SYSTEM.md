...
> 创建：2026-08-13　|　修订：2026-08-19（Phase 3 已实施）
> 状态：Phase 0/1/2 已实施 ✅；Phase 3 已实施 ✅（Play Mode 验证待玩家执行）
...
ItemType {
    ...
    PeaSeed,           // 豌豆粒（分解产物，携带基因，可种植为 PeaStem）
...
- 分解 ✅（2026-08-19）：手持右键 / 背包窗右键 → 4~8 粒豌豆种子（带各自母本基因）
...
### Phase 3 — 分解 ✅（2026-08-19，见 WORKLOG_2026-08-19）
- ✅ 手持右键分解豌豆荚（选中豌豆荚时右键，或背包窗内右键豌豆荚行）
- ✅ 每荚 → 4~8 粒豌豆种子（携带母本基因组 + 采收基因 HTT 载荷）
- ✅ 优先存入种子袋，无种子袋/满则落入背包（豌豆粒可堆叠、可种植）
- ✅ 豌豆粒（`ItemType.PeaSeed`）可放置为 `PeaStem`，种植继承母本基因 + 采收基因，消耗 1 粒
- ✅ 种子袋子面板新增「取出」按钮（按基因型整组取出落入背包）
- ✅ 修复存档缺口：豌豆荚/豌豆粒的子集表型标签读档后保留（堆叠分组不失效）
- ⏳ Play Mode 验证（玩家执行）
