# 贴图随机旋转方案（讨论稿，2026-08-13）

> 状态：**规划中（未实施）**。本文档记录"贴图朝向随机化"的候选方案与实施计划，供实施时参考。
> 关联：`docs/status/WORKLOG_2026-08-13.md`、`AGENTS.md`（Atlas/UV 数学约定）、`MeshData.cs`/`ChunkMeshBuilder.cs`（渲染管线）。
...
旋转 90° 的倍数的**唯一实现点就是 UV**：

- `AddFace(x, y, z, dir, block)` 增加 `int rotation`（0=0° 1=90° 2=180° 3=270°）参数，`GetFaceUVs` 对 4 个 UV 角点做 `rotation` 次循环位移。
- `AddPeaQuadCell(x, y, z, cell)` 同样增加 `rotation` 参数，对 `uv` 数组做循环位移。

**安全性**：图集 cell 是 24px（16px 内容 + 四周 4px padding）。UV 4 角始终落在 `[u0, u0+s] × [v0, v0+s]` 的 16×16 内容区内，旋转只是角点重排，**采样范围不越出内容区**，不会采到 padding 或相邻 cell——与 AGENTS.md 的 24px-cell 公式约束无冲突。

**注意**：豌豆十字面片是 X/Z 两对角交叉（`AddPeaQuadCell` 的 quad a/b），旋转 90° 后两片互换但交叉形状不变——所以豌豆的"朝向"表现为**贴图内容旋转**（豆荚/花朵的方向变化），几何本身对称。这是符合"消除重复"目标的预期行为。
...
| 方块/面 | 是否旋转 | 说明 |
|---|---|---|
| PeaStem / PeaPlantTop（十字面片） | ✅ | 阶段 2/3 的植株、阶段 3 的荚 |
| 草方块顶面（Grass, Up） | ✅ | 用户点名 |
| 树叶（Leaves） | ✅（可选） | 有斑驳方向感 |
...
阶段 2/3 是两格高植株：底部格 `PeaStem` + 顶部格 `PeaPlantTop`，两格**可能不在同一 chunk**（y=15 底 + y=0 顶）。为保证同株两格贴图朝向一致：
