...
| 步骤 | 内容 | 状态 |
|------|------|------|
| Step 0（前置） | 修复 `SimpleRegionWriter` 用 `FileMode.Create` 整文件重建隐患 → 改 `FileMode.OpenOrCreate` + 读旧索引续写；否则遗传数据会不可逆丢失 | ⏳ |
...
| 2c | `Inventory/ItemInstance.cs` + `Inventory/Backpack.cs`：物品实例（itemType/genome/双标签/标准化映射），非堆叠列表 | ⏳ |
...
| 2f | `UI/BackpackWindow.cs`（F 开关）+ `UI/TagEditorWindow.cs`（双分区异色）+ `UI/StandardizeWindow.cs`（F9 调试向导） | ⏳ |
